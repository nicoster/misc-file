...
import org.ocp.media.DataSource;
import org.ocp.media.DiskCache;
import org.ocp.media.Gallery;
import org.ocp.media.LocalDataSource;
import org.ocp.media.LongSparseArray;
import org.ocp.media.MediaFeed;
import org.ocp.media.MediaItem;
import org.ocp.media.MediaSet;
import org.ocp.media.Shared;
import org.ocp.media.SortCursor;
import org.ocp.media.Utils;
...
public final class CacheService extends IntentService {
	public static final String ACTION_CACHE = "org.ocp.cache.action.CACHE";
	...
	public static final String getCachePath(final String subFolderName) {
		return Environment.getExternalStorageDirectory()
				+ "/Android/data/org.ocp.media/cache/" + subFolderName;
	}
	...
	public static final boolean isCacheReady(final boolean onlyMediaSets) {
		if (onlyMediaSets) {
			return (sAlbumCache.get(ALBUM_CACHE_METADATA_INDEX, 0) != null && sAlbumCache
					.get(ALBUM_CACHE_DIRTY_INDEX, 0) == null);
		} else {
			return (sAlbumCache.get(ALBUM_CACHE_METADATA_INDEX, 0) != null
	...
	public static final boolean setHasItems(final ContentResolver cr,
		...
		try {
			...
			final Cursor cursorVideos = cr.query(uriVideos,
					BUCKET_PROJECTION_VIDEOS, whereString.toString(), null,
					null);
	...
	public static final void loadMediaSets(final Context context,
			final MediaFeed feed, final DataSource source,
		...
		if (albumData != null && albumData.length > 0) {
			...
			try {
				...
				for (int i = 0; i < numAlbums; ++i) {
					...
					if (mediaSet == null) {
                        Log.i(Gallery.TAG, TAG + ":loadMediaSets:setId=" + setId);
				...
				Log.e(Gallery.TAG, TAG + ": " + "Error loading albums.");
				sAlbumCache.deleteAll();
				putLocaleForAlbumCache(Locale.getDefault());
			}
		} else {

			Log.d(Gallery.TAG, TAG + ": " + "No albums found.");
		}
	}

	public static final void loadMediaSet(final Context context,
			final MediaFeed feed, final DataSource source, final long bucketId) {
		syncCache(context);
		final byte[] albumData = sAlbumCache.get(ALBUM_CACHE_METADATA_INDEX, 0);
		if (albumData != null && albumData.length > 0) {
			...
			try {
				...
				for (int i = 0; i < numAlbums; ++i) {
					...
					if (setId == bucketId) {
						...
						if (mediaSet == null) {
                            Log.i(Gallery.TAG, TAG + ":loadMediaSet:setId=" + setId);
							mediaSet = feed.addMediaSet(setId, source);
						}
					} else {
				...
				Log.e(Gallery.TAG, TAG + ": " + "Error finding album "
						+ bucketId);
				sAlbumCache.deleteAll();
				putLocaleForAlbumCache(Locale.getDefault());
			}
			...
			Log.d(Gallery.TAG, TAG + ": " + "No album found for album id "
					+ bucketId);
		}
	}

	public static final void loadMediaItemsIntoMediaFeed(final Context context,
			final MediaFeed feed, final MediaSet set, final int rangeStart,
			final int rangeEnd, final boolean includeImages,
			final boolean includeVideos) {
        Log.d(Gallery.TAG, TAG + ":loadMediaItemsIntoMediaFeed:");
		syncCache(context);
		...
		if (albumData != null
			...
			try {
				...
				for (int i = 0; i < numItems; ++i) {
					MediaItem item = (reuseItem == null) ? new MediaItem()
							: reuseItem;
					...
					boolean setLookupContainsItem = set
							.lookupContainsItem(item);
				...
				Log.e(Gallery.TAG, TAG + ": "
						+ "Error loading items for album " + set.mName);
				sAlbumCache.deleteAll();
				putLocaleForAlbumCache(Locale.getDefault());
			}
		} else {

			Log.d(Gallery.TAG, TAG + ": " + "No items found for album "
					+ set.mName);
		}
	...
	private static void syncCache(Context context) {
		if (!isCacheReady(true)) {
			...
			if ((context instanceof Gallery) && !Gallery.isShuttingDown()) {
				App.get(context).showToast(
						context.getResources()
								.getString(Res.string.loading_new),
						Toast.LENGTH_LONG);
			}

			Log.d(Gallery.TAG, TAG + ": " + "Refreshing Cache for all items");
			refresh(context);
			sAlbumCache.delete(ALBUM_CACHE_DIRTY_INDEX);
			sAlbumCache.delete(ALBUM_CACHE_DIRTY_BUCKET_INDEX);
		} else if (!isCacheReady(false)) {

			Log.d(Gallery.TAG, TAG + ": " + "Refreshing Cache for dirty items");
			refreshDirtySets(context);
			sAlbumCache.delete(ALBUM_CACHE_DIRTY_BUCKET_INDEX);
		}
	}

	public static final void populateVideoItemFromCursor(final MediaItem item,
			final ContentResolver cr, final Cursor cursor, final String baseUri) {
		item.setMediaType(MediaItem.MEDIA_TYPE_VIDEO);
		populateMediaItemFromCursor(item, cr, cursor, baseUri);
	}

	public static final void populateMediaItemFromCursor(final MediaItem item,
			final ContentResolver cr, final Cursor cursor, final String baseUri) {
	...
	public static final long fetchDateTaken(final MediaItem item) {
		if (!item.isDateTakenValid()
			...
			try {
				...
				Log.i(Gallery.TAG, TAG + ": " + "Parsing date taken from exif");
	...
	public static final ImageList getImageList(final Context context) {
		...
		try {
			final Cursor cursorImages = cr.query(uriImages,
					THUMBNAIL_PROJECTION, null, null, null);
			if (cursorImages != null && cursorImages.moveToFirst()) {
				...
				do {
					...
					thumbnailIds[ctr] = Utils.Crc64Long(cursorImages
	...
	private static final byte[] queryThumbnail(final Context context,
		...
		if (bitmap == null) {
			...
			Log.i(Gallery.TAG, TAG + ": "
					+ "Built thumbnail and screennail for " + origId + " in "
	...
	private static final void buildThumbnails(final Context context) {
		...
		Log.i(Gallery.TAG, TAG + ": "
				+ "Preparing DiskCache for all thumbnails.");
		...
		Log.i(Gallery.TAG, TAG + ": " + "DiskCache ready for all thumbnails.");
	}
	...
	private static boolean isInThumbnailerSkipList(long thumbnailId) {
		if (sSkipThumbnailIds != null
				&& sSkipThumbnailIds.isDataAvailable(thumbnailId, 0)) {
	...
	protected void onHandleIntent(final Intent intent) {
		...
		Log.i(Gallery.TAG, TAG + ": " + "Starting CacheService");
	...
	private static final void putLocaleForAlbumCache(final Locale locale) {
		...
		try {
			...
			Log.i(Gallery.TAG, TAG + ": " + "Error writing locale to cache.");
			;
		}
	}
	...
	private static final Locale getLocaleForAlbumCache() {
		...
		if (data != null && data.length > 0) {
			...
			try {
				...
				Log.i(Gallery.TAG, TAG + ": "
						+ "Error reading locale from cache.");
				return null;
			}
		}
		return null;
	...
	private final static void refresh(final Context context) {
		...
		Log.i(Gallery.TAG, TAG + ": " + "Refreshing cache.");
		synchronized (sCacheLock) {
			sAlbumCache.deleteAll();
			putLocaleForAlbumCache(Locale.getDefault());
			...
			Log.i(Gallery.TAG, TAG + ": " + "Building albums.");
			...
			final ContentResolver cr = context.getContentResolver();
			try {
                String where = Images.ImageColumns.DATA + " LIKE '/mnt/sdcard/Dishes/%'";
                final Cursor cursorImages = cr.query(uriImages, BUCKET_PROJECTION_IMAGES, where, null,
                        DEFAULT_BUCKET_SORT_ORDER);
                final Cursor cursorVideos = cr.query(uriVideos, BUCKET_PROJECTION_VIDEOS, where, null,
                        DEFAULT_BUCKET_SORT_ORDER);
				...
				Log.i(Gallery.TAG, TAG + ": " + "Done building albums.");
		...
		synchronized (sCacheLock) {
			...
			if (existingData != null && existingData.length > 0) {
				...
				if (numIds > 0) {
					...
					Log.i(Gallery.TAG, TAG + ": " + "Refreshing dirty albums");
					populateMediaItemsForSets(context, sets, acceleratedSets,
							true);
				}
			}
			sAlbumCache.delete(ALBUM_CACHE_DIRTY_BUCKET_INDEX);
		...
		try {
			...
			try {
				...
				if (numSets > 0) {
					...
					final byte[] albumData = sAlbumCache.get(
							ALBUM_CACHE_METADATA_INDEX, 0);
					if (albumData != null && albumData.length > 0) {
						final DataInputStream dis = new DataInputStream(
								new BufferedInputStream(
										new ByteArrayInputStream(albumData),
										256));
						try {
							final int numAlbums = dis.readInt();
							for (int i = 0; i < numAlbums; ++i) {
	...
	private final static void populateMediaItemsForSets(final Context context,
		...
		Log.i(Gallery.TAG, TAG + ": " + "Building items.");
		final Uri uriImages = Images.Media.EXTERNAL_CONTENT_URI;
		final Uri uriVideos = Video.Media.EXTERNAL_CONTENT_URI;
		final ContentResolver cr = context.getContentResolver();
		...
		if (useWhere) {
			...
			Log.i(Gallery.TAG, TAG + ": " + "Updating dirty albums where "
					+ whereClause);
		}
		try {
				...
				if (sortCursor != null && sortCursor.moveToFirst()) {
					...
					do {
						...
						final boolean isVideo = (sortCursor
								.getCurrentCursorIndex() == 1);
						if (isVideo) {
							populateVideoItemFromCursor(item, cr, sortCursor,
									CacheService.BASE_CONTENT_STRING_VIDEOS);
						} else {
							populateMediaItemFromCursor(item, cr, sortCursor,
		...
		if (sets.size() > 0) {
			...
			Log.i(Gallery.TAG, TAG + ": " + "Done building items.");
		}
	}
	...
	private static final void writeSetsToCache(final ArrayList<MediaSet> sets) {
		...
		try {
			...
			Log.e(Gallery.TAG, TAG + ": "
					+ "Error writing albums to diskcache.");
			sAlbumCache.deleteAll();
			putLocaleForAlbumCache(Locale.getDefault());
		}
	}
	...
	private static final void writeItemsForASet(final MediaSet set) {
		...
		try {
			...
			Log.e(Gallery.TAG, TAG + ": "
					+ "Error writing to diskcache for set " + set.mName);
			sAlbumCache.deleteAll();
			putLocaleForAlbumCache(Locale.getDefault());
		}
	}
