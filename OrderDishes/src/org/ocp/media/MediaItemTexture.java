    ...
    protected Bitmap load(RenderView view) {
        ...
        if (config == null) {
            ...
            try {
                if (mItem.getMediaType() == MediaItem.MEDIA_TYPE_IMAGE) {
                    ...
                    new Thread() {
                        public void run() {
                            ...
                            try {
                                MediaStore.Video.Thumbnails.cancelThumbnailRequest(mContext.getContentResolver(),
                                        mItem.mId);
                            } catch (Exception e) {
                                ;
                            }
                ...
                Log.i(Gallery.TAG, TAG + ": " + "Bitmap creation fail, outofmemory");
            ...
            if (parentMediaSet != null && !parentMediaSet.mIsLocal) {
                ...
                if (data == null) {
                    ...
                    try {
                        ...
                        data = CacheService.writeBitmapToCache(thumbnailCache, item.mId, item.mId, retVal,
                                config.thumbnailWidth, config.thumbnailHeight, item.mDateModifiedInSec * 1000);
                    } catch (IOException e) {
                        return null;
                    } catch (URISyntaxException e) {
                        return null;
            ...
            if (data != null) {
                try {
                    ...
                    final Bitmap bitmap = BitmapFactory.decodeByteArray(data, CACHE_HEADER_SIZE, data.length
                            - CACHE_HEADER_SIZE, options);
                    return bitmap;
                } catch (IOException e) {
