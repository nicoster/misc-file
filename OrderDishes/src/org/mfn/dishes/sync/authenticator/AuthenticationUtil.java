...
public class AuthenticationUtil {
	...
	public synchronized static void saveNewAccount(Context context) {
		...
		final Account account = new Account(mAccount.accountName,
				Constants.ACCOUNT_TYPE);

		Bundle userData = new Bundle();
		userData.putString(Constants.PARAM_SERVER_ADDRESS,
				mAccount.serverAddress);
		userData.putString(Constants.PARAM_SERVER_PORT, mAccount.serverPort);
		mAccountManager.addAccountExplicitly(account, mAccount.password,
				userData);
	...
	public static ImAccount getmAccount() {
		return mAccount;
	}
	...
	public synchronized static void saveEditAccount(Context context,
		...
		mAccountManager.setUserData(account, Constants.PARAM_SERVER_ADDRESS,
				mAccount.serverAddress);
		mAccountManager.setUserData(account, Constants.PARAM_SERVER_PORT,
				mAccount.serverPort);
	...
	private static void handleSyncContacts(final Account account,
			ContentResolver resolver, ImAccount mAccount) {
		Log.i(TAG, "handleSyncContacts:");
		ContentResolver.setSyncAutomatically(account,
				ContactsContract.AUTHORITY, mAccount.syncContacts);
		...
		ContentResolver.setIsSyncable(account, ContactsContract.AUTHORITY,
				mAccount.syncContacts ? 1 : 0);
		final int contactsVisibility = getContactsVisibility(resolver,
	...
	private static int getContactsVisibility(final ContentResolver resolver,
		...
		final Uri settingsUri = Settings.CONTENT_URI.buildUpon()
				...
				.build();
		final Cursor cursor = resolver.query(settingsUri,
				new String[]{Settings.UNGROUPED_VISIBLE}, null, null, null);
