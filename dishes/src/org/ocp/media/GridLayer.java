...
public final class GridLayer extends RootLayer implements MediaFeed.Listener, TimeBar.Listener {
    ...
    public GridLayer(Context context, int itemWidth, int itemHeight, LayoutInterface layoutInterface, RenderView view) {
        ...
        mHud.getPathBar().pushLabel(Res.drawable.icon_home_small,
                context.getResources().getString(Res.string.app_name), new Runnable() {
                    public void run() {
    ...
    public void setDataSource(DataSource dataSource) {
        Log.i(Gallery.TAG, TAG + ":setDataSource:" + dataSource.getClass().getName());
        MediaFeed feed = mMediaFeed;
        mMediaFeed = new MediaFeed(mContext, dataSource, this);
        if (feed != null) {
            // Restore the slot state in the original feed before shutting it
            // down.
    ...
    private int hitTest(Vector3f worldPos, int itemWidth, int itemHeight) {
        ...
        try {
            ...
            for (int i = firstSlotIndex; i <= lastSlotIndex; ++i) {
                GridCameraManager.getSlotPositionForSlotIndex(i, mCamera, mLayoutInterface, deltaAnchorPosition,
                        position);
                if (FloatUtils.boundsContainsPoint(position.x - itemWidthBy2, position.x + itemWidthBy2, position.y
                        - itemHeightBy2, position.y + itemHeightBy2, worldPos.x, worldPos.y)) {
                    retVal = i;
                    break;
                }
            }
    ...
    void centerCameraForSlot(int slotIndex, float baseConvergence, boolean looping) {
        float imageTheta = 0.0f;
        DisplayItem displayItem = getDisplayItemForSlotId(slotIndex);
        if (displayItem != null) {
            imageTheta = displayItem.getImageTheta();
        }
        mCameraManager.centerCameraForSlot(mLayoutInterface, slotIndex, baseConvergence,
                mDeltaAnchorPositionUncommited, mInputProcessor.getCurrentSelectedSlot(), mZoomValue, imageTheta,
                mState, looping);
    }
    ...
    public boolean update(RenderView view, float timeElapsed) {
        ...
        // isSingleImageMode() returns true only when the item is not found in
        // DB.
        ...
        if (view.elapsedLoadingExpensiveTextures() > 150
                || (mMediaFeed != null && mMediaFeed.getWaitingForMediaScanner())) {
        ...
        if (mDisplayList.getNumAnimatables() != 0 || mCamera.isAnimating() || mSelectedAlpha != mTargetAlpha
    ...
    private void computeVisibleItems() {
        ...
        if (deltaBegin != 0 || deltaEnd != 0) {
                ...
                for (int i = firstVisibleSlotIndex; i <= lastVisibleSlotIndex; ++i) {
                    ...
                    if (set != null && indexIntoSlots >= 0 && indexIntoSlots < numDisplaySlots) {
                        ...
                        ArrayList<MediaItem> bestItems = mTempList;
                        {
                            ...
                            // sTempHash);
                            // else
                            ArrayUtils.computeSortedIntersection(visibleItems, items, MAX_ITEMS_PER_SLOT, bestItems,
                                    mTempHash);
                        }
                                ...
                                if (itemUri != null && mRequestFocusContentUri != null) {
                                    if (itemUri.equals(mRequestFocusContentUri)) {
                                        if (mState == STATE_FULL_SCREEN) {
                                            ...
                                            centerCameraForSlot(i, 1.0f, false);
                                        }
                                        break;
                                    }
                                }
            ...
            int startMemoryRange = (mBufferedVisibleRange.begin / numThumbnailsToKeepInMemory)
                    * numThumbnailsToKeepInMemory;
    ...
    public void renderOpaque(RenderView view, GL11 gl) {
        ...
        gl.glLoadIdentity();
        GLU.gluLookAt(gl, -camera.mEyeX, -camera.mEyeY, -camera.mEyeZ, -camera.mLookAtX, -camera.mLookAtY,
        ...
        if (mHud != null && mDrawManager != null) {
            if (mMediaFeed != null) {
                mDrawManager.drawBlendedComponents(view, gl, mSelectedAlpha, mState, mHud.getMode(),
                        mTimeElapsedSinceStackViewReady, mTimeElapsedSinceGridViewReady, mSelectedBucketList,
                        mMarkedBucketList,
    ...
    public synchronized void onLayout(int newAnchorSlotIndex, int currentAnchorSlotIndex, LayoutInterface oldLayout) {
        ...
        centerCameraForSlot(newAnchorSlotIndex, 1.0f, false);
    ...
    public synchronized void onFeedChanged(MediaFeed feed, boolean needsLayout) {
        ...
        Log.i(Gallery.TAG, TAG + ": " + "Slot changing from " + currentlyVisibleSlotIndex + " to " + newSlotIndex);
    ...
    boolean changeFocusToNextSlot(float convergence) {
        int currentSelectedSlot = mInputProcessor.getCurrentSelectedSlot();
        return changeFocusToSlot(currentSelectedSlot + 1, convergence);
    }

    boolean changeFocusToSlot(int slotId, float convergence) {
        mZoomValue = 1.0f;

        if (slotId < mCompleteRange.begin) {
            slotId = mCompleteRange.end;
            mInputProcessor.setCurrentFocusSlot(slotId);
            centerCameraForSlot(slotId, convergence, true);
            computeVisibleItems();
        }
        if (slotId > mCompleteRange.end) {
            slotId = mCompleteRange.begin;
            mInputProcessor.setCurrentFocusSlot(slotId);
            centerCameraForSlot(slotId, convergence, true);
            computeVisibleItems();
        }
        ...
        if (index >= 0 && slotId <= mBufferedVisibleRange.end) {
            ...
            if (displayItem != null) {
                ...
                if (slotId != Shared.INVALID && slotId <= mCompleteRange.end) {
                    mInputProcessor.setCurrentFocusSlot(slotId);
                    centerCameraForSlot(slotId, convergence, false);
                    return true;
                } else {
                    centerCameraForSlot(mInputProcessor.getCurrentSelectedSlot(), convergence, false);
                    return false;
                }
            }
        }
        ...
        if (!feed.isClustered()) {
            ...
            if (metadata) {
                ...
                if (slot.hasValidLocation()) {
                    ...
                    feed.setFilter(new LocationMediaFilter(set.mMinLatLatitude, set.mMinLonLongitude,
                            set.mMaxLatLatitude, set.mMaxLonLongitude));
                }
            }
            return false;
        }
    ...
    public void startSlideshow() {
        ...
        mSlideshowMode = true;
        mZoomValue = 1.0f;
        centerCameraForSlot(mInputProcessor.getCurrentSelectedSlot(), 1.0f, false);
    ...
    private float getFillScreenZoomValue() {
        return GridCameraManager.getFillScreenZoomValue(mCamera, mTempVec, mCurrentFocusItemWidth,
    ...
    public void setPickIntent(boolean b) {
        mPickIntent = b;
        mHud.getPathBar().popLabel();
        mHud.getPathBar().pushLabel(Res.drawable.icon_location_small,
                mContext.getResources().getString(Res.string.pick), new Runnable() {
                    public void run() {
                        if (mHud.getAlpha() == 1.0f) {
                            if (!mFeedAboutToChange) {
                                setState(STATE_MEDIA_SETS);
