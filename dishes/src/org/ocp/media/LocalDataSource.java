...
public class LocalDataSource implements DataSource {
    ...
    public LocalDataSource(final Context context, final String uri, final boolean flattenAllItems) {
        Log.w(Gallery.TAG, TAG + ":LocalDataSource:uri=" + uri);
        this.mUri = uri;
        mContext = context;
        mIncludeImages = true;
        ...
        mDiskCache = mUri.startsWith(MediaStore.Images.Media.EXTERNAL_CONTENT_URI.toString())
                ...
                || mUri.startsWith("file://") ? sThumbnailCache : null;
    }
    ...
    public void loadItemsForSet(MediaFeed feed, MediaSet parentSet, int rangeStart, int rangeEnd) {
        Log.i(Gallery.TAG, TAG + ":loadItemsForSet:" + feed.toString() + "\tMediaSet=" + parentSet.mName);
            ...
            if (item != null) {
                ...
                if (mUri.startsWith("file://")) {
                    try {
                        ...
                        item.mRotation = Shared.exifOrientationToDegrees(exif.getAttributeInt(
                                ExifInterface.TAG_ORIENTATION, ExifInterface.ORIENTATION_NORMAL));
                    } catch (IOException e) {
                        Log.i(Gallery.TAG, TAG + ": " + "Error reading Exif information, probably not a jpeg.");
                    }
                }
            ...
            try {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    do {
                        ...
                        CacheService.populateMediaItemFromCursor(item, cr, cursor,
    ...
    public void loadMediaSets(final MediaFeed feed) {
        Log.d(Gallery.TAG, TAG + "#:loadMediaSets:" + feed.getMediaSets().size());
        ...
        if (mSingleUri) {
            ...
            Log.i(Gallery.TAG, TAG + ":loadMediaSets:id=" + id);
        ...
        if (!mAllItems && set != null && loadOtherSets) {
            ...
            // not re-ordering media sets in the case of displaying a single
            // image
            if (!mSingleUri) {
    ...
    public boolean performOperation(int operation, ArrayList<MediaBucket> mediaBuckets, Object data) {
        Log.i(Gallery.TAG, TAG + ":performOperation");
        int numBuckets = mediaBuckets.size();
        ContentResolver cr = mContext.getContentResolver();
        switch (operation) {
        case MediaFeed.OPERATION_DELETE:
            Log.i(Gallery.TAG, TAG + ":performOperation:MediaFeed.OPERATION_DELETE");
        ...
        case MediaFeed.OPERATION_ROTATE:
            Log.i(Gallery.TAG, TAG + ":performOperation:MediaFeed.OPERATION_ROTATE");            
            for (int i = 0; i < numBuckets; ++i) {
                MediaBucket bucket = mediaBuckets.get(i);
                ArrayList<MediaItem> items = bucket.mediaItems;
                if (items == null) {
    ...
    private void rotateItem(final MediaItem item, float angleToRotate) {
        Log.i(Gallery.TAG, TAG + ":rotateItem:" + item.mFilePath);
        ContentResolver cr = mContext.getContentResolver();
        try {
            ...
            if (uriScheme.equals("file") || uriScheme.equals("content")) {
                ...
                exif.setAttribute(ExifInterface.TAG_ORIENTATION,
                        Integer.toString(Shared.degreesToExifOrientation(rotation)));
                exif.saveAttributes();
            }
    ...
    public static MediaItem createMediaItemFromUri(Context context, Uri target, int mediaType) {
        Log.d(Gallery.TAG, TAG + ":createMediaItemFromUri:" + target.getPath());
        MediaItem item = null;
        long id = ContentUris.parseId(target);
        ContentResolver cr = context.getContentResolver();
        String whereClause = Images.ImageColumns._ID + "=" + Long.toString(id);
        try {
            final Uri uri = (mediaType == MediaItem.MEDIA_TYPE_IMAGE) ? Images.Media.EXTERNAL_CONTENT_URI
                    : Video.Media.EXTERNAL_CONTENT_URI;
            final String[] projection = (mediaType == MediaItem.MEDIA_TYPE_IMAGE) ? CacheService.PROJECTION_IMAGES
    ...
    public static MediaItem createMediaItemFromFileUri(Context context, String fileUri) {
        Log.d(Gallery.TAG, TAG + ":createMediaItemFromFileUri:" + fileUri);
        MediaItem item = null;
        ...
        String whereClause = Images.ImageColumns.BUCKET_ID + "=" + bucketId + " AND " + Images.ImageColumns.DATA + "='"
                + filepath + "'";
        Log.d(Gallery.TAG, TAG + ":createMediaItemFromFileUri" + whereClause);
        try {
            Cursor cursor = cr.query(Images.Media.EXTERNAL_CONTENT_URI, CacheService.PROJECTION_IMAGES, whereClause,
                    null, null);
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    item = new MediaItem();
    ...
    public void refresh(final MediaFeed feed, final String[] databaseUris) {
        ...
        for (int i = 0; i < numDirtySets; ++i) {
            long setId = ids[i];
            Log.i(Gallery.TAG, TAG + ":refresh:setId=" + setId);
