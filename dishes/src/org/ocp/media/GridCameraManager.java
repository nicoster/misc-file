...
public final class GridCameraManager {
    ...
    public void centerCameraForSlot(LayoutInterface layout, int slotIndex, float baseConvergence, Vector3f deltaAnchorPositionIn,
            int selectedSlotIndex, float zoomValue, float imageTheta, int state, boolean looping) {
        ...
        synchronized (camera) {
            ...
            if (slotIndex >= 0) {
                ...
                try {
                    ...
                    if (looping) {
                    	camera.moveToAbsolute(position.x, position.y, zoomin ? camera.getDistanceToFitRect(width * 
                    			oneByZoom, height * oneByZoom) : 0);
                    } else {
                    	camera.moveTo(position.x, position.y, zoomin ? camera.getDistanceToFitRect(width * 
                    			oneByZoom, height * oneByZoom) : 0);
                    }
                } finally {
    ...
    public void computeVisibleRange(MediaFeed feed, LayoutInterface layout, Vector3f deltaAnchorPositionIn,
        ...
        try {
            ...
            int range = lastVisibleSlotIndex - firstVisibleSlotIndex;
            if( range >= GridLayer.MAX_DISPLAY_SLOTS){
                firstVisibleSlotIndex += (GridLayer.MAX_DISPLAY_SLOTS - range) + 1;
            }
