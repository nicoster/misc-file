...
int _tmain( int argc, TCHAR* argv[] ) 
{
	...
	string pdbsig;
	GetPdbSignature(pdbsig);
	
	string url = _T("http://msdl.microsoft.com/download/symbols/wininet.pdb/");
	url += pdbsig + _T("/wininet.pd_");
	cout << url << endl;
	return 0;
}
...
//		DumpDebugDirectoryEntries( (LPBYTE)lpFileMem, pDebugDir, DebugDirSize ); 
		LPBYTE pImageBase = (LPBYTE)lpFileMem;
		if( !CheckDebugDirectory( pDebugDir, DebugDirSize ) ) 
		{
			_ASSERT( 0 ); 
			break; 
...
#endif					
					_tsprintf(buf, _T("%08X%04X%04X%02X%02X%02X%02X%02X%02X%02X%02X%d"), 
