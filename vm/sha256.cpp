...
void SHA256Finalize(SHA256Context *sha,
					  unsigned char hash[_SHA256_HASH_SIZE]);
 ...
 */
#if SIZEOF_LONG > 4
	#define	TRUNCLONG(x)	((x) & __MAX_UINT32)
...
static void ProcessBlock(SHA256Context *sha, const unsigned char *block)
{
	...
	for(t = 0; t < 16; ++t)
	{
		W[t] = (((_UInt32)(block[t * 4 + 0])) << 24) |
		       (((_UInt32)(block[t * 4 + 1])) << 16) |
		       (((_UInt32)(block[t * 4 + 2])) <<  8) |
		        ((_UInt32)(block[t * 4 + 3]));
	}
	for(t = 16; t < 64; ++t)
	{
 ...
 */
static void WriteLong(unsigned char *buf, _UInt32 value)
{
					  ...
					  unsigned char hash[_SHA256_HASH_SIZE])
{
	...
	if(hash)
	{
		...
		WriteLong(sha->input + 56, (_UInt32)(totalBits >> 32));
		WriteLong(sha->input + 60, (_UInt32)totalBits);
		ProcessBlock(sha, sha->input);
...
static void ProcessVector(SHATestVector *vector)
{
	...
	/* Compute the hash */
	SHA256Init(&sha);
	SHA256Data(&sha, vector->value, strlen(vector->value));
