...
- (UITextField *)create_UITextFieldWithDictionary:(NSDictionary *)dict
{
	...
	tf.tag = [(NSNumber*)[dict objectForKey:@"tag"] integerValue];
...
{
	NSString *val = [NSString stringWithFormat: @"title4section%d", section];
	val = [[NSUserDefaults standardUserDefaults] stringForKey:val];
	if (val.length) {
		return val;
	}
